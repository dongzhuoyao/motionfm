...
class TrainLoop_Diffusion:
    def __init__(self, args, train_platform, model, dynamic, data_loader,fixed_noise):
        self.args = args
        ...
        self.resume_checkpoint = args.training.resume_checkpoint
        ...
        self.save_dir = args.training.save_dir
        self.overwrite = args.training.overwrite
        ...
        self.schedule_sampler = create_named_schedule_sampler(
            self.schedule_sampler_type, dynamic
        )
        self.eval_wrapper, self.eval_data, self.eval_gt_data = None, None, None
        if args.dataset in ["kit", "humanml"] and args.training.eval_during_training:
            mm_num_samples = 0  # mm is super slow hence we won't run it during training
            mm_num_repeats = 0  # mm is super slow hence we won't run it during training
            gen_loader = get_dataset_loader(
                name=args.dataset,
                batch_size=args.trainig.eval_batch_size,
                num_frames=None,
                split=args.training.eval_split,
                hml_mode="eval",
            )

            self.eval_gt_data = get_dataset_loader(
                name=args.dataset,
                batch_size=args.training.eval_batch_size,
                num_frames=None,
                split=args.training.eval_split,
                hml_mode="gt",
            )
    ...
    def evaluate(self):
        if not self.args.training.eval_during_training:
            return
        ...
        if self.eval_wrapper is not None:
            ...
            eval_dict = eval_humanml.evaluation(
                ...
                self.eval_data,
                log_file,
                replication_times=self.args.training.eval_rep_times,
            ...
            eval_args = SimpleNamespace(
                num_seeds=self.args.training.eval_rep_times,
                num_samples=self.args.training.eval_num_samples,
                batch_size=self.args.training.eval_batch_size,
                device=self.device,
                guidance_param=1,
                dataset=self.dataset,
                unconstrained=self.args.model.unconstrained,
    ...
    def forward_backward(self, batch, cond):
        ...
        for i in range(0, batch.shape[0], self.microbatch):
            ...
            log_loss_dict(self.dynamic, t, {k: v * weights for k, v in losses.items()})
