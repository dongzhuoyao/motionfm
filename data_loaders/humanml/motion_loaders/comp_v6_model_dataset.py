...
class CompMDMGeneratedDataset(Dataset):
    def __init__(
        ...
        diffusion_steps_sample = cfg.diffusion_steps_sample
        ...
        print("random selected mm_idxs", mm_idxs)

        model.eval()

        with torch.no_grad():
            for _idx, (motion, model_kwargs) in tqdm(
                enumerate(dataloader),
                total=len(dataloader),
                desc="loading generated_motion",
            ):
                if (
                ...
                if scale != 1.0:
                    ...
                    model_kwargs["y"]["scale"] = (
                        torch.ones(len(motion), device=dist_util.dev()) * scale
                    )

                is_mm = _idx in mm_idxs
                repeat_times = mm_num_repeats if is_mm else 1
                mm_motions = []
                for t in range(repeat_times):
                    sample = sample_fn(
                        ...
                        sample_steps=diffusion_steps_sample,
