    ...
    def __init__(
        ...
        with torch.no_grad():
            for motions, model_kwargs in tqdm(
                ...
                if (
                    num_samples != -1
                    and len(self.batches) * dataiterator.batch_size > num_samples
                ):
                ...
                mask = (
                    model_kwargs["y"]["mask"]
                    .reshape(dataiterator.batch_size, max_n_frames)
                    .bool()
                )
                batch["output_xyz"] = model.rot2xyz(
                    x=batch["output"],
                    mask=mask,
                    pose_rep="rot6d",
                    glob=True,
                ...
                if not unconstrained:  # proceed only if not running unconstrained
                    batch["y"] = (
                        model_kwargs["y"]["action"].squeeze().long().cpu()
...
def evaluate(args, model, diffusion, data, cfg):
    num_frames = 60

    # fix parameters for action2motion evaluation
    args.num_frames = num_frames  # flag: sssdsdhere
    ...
    try:
        for index, seed in enumerate(allseeds):
            ...
            new_data_loader = functools.partial(
                NewDataloader,
                model=model,
                diffusion=diffusion,
                device=device,
            ...
            motionloader = new_data_loader(
                mode="gen", dataiterator=dataiterator, cfg=cfg
            )
        ...
        if args.unconstrained:  # unconstrained
            ...
            dataiterator_unconstrained = DataLoader(
                dataset_unconstrained,
                batch_size=args.batch_size,
                shuffle=False,
                num_workers=8,
                collate_fn=collate,
            )
            motionloader_unconstrained = new_data_loader(
                mode="gen",
                dataiterator=dataiterator_unconstrained,
            ...
            unconstrained_metrics = evaluate_unconstrained_metrics(
                generated_motions, device, fast=True
            )
            unconstrained_metrics = {
                k + "_unconstrained": v for k, v in unconstrained_metrics.items()
            }
