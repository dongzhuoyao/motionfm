...
class GaussianDiffusion:
    ...
    def p_sample_loop_progressive(
        ...
        if sample_steps is not None:
            sparse_indx = (
                np.linspace(0, 1, sample_steps, endpoint=False) * self.num_timesteps
            )
            sparse_indx = np.round(sparse_indx).astype(int)
            indices = [indices[i] for i in sparse_indx]
